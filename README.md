...
GitHub Pages cannot run the Node/Express backend. To make Prompt Saver deployable there, the repository also includes a static build in [docs/](C:/Users/liamn/Documents/Codex/2026-04-23-be-extremely-careful-not-to-turn/prompt-saver/docs).

How Pages mode works:

- the UI runs fully as a static site
- the user pastes their own Gemini API key into the page
- the key is stored only in the browser's local storage on that device
- the browser calls the Gemini REST API directly

Important tradeoff:

- this is suitable for personal use or testing
- it is not as secure as the server-backed version because client-side API keys can be extracted

Google's official Gemini API key guidance says server-side calls are the most secure option, and client-side keys can be extracted. Source: [Using Gemini API keys](https://ai.google.dev/gemini-api/docs/api-key?authuser=9)
